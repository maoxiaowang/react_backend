...
from react_backend.settings import BASE_DIR

_PERMS_DIR = Path(BASE_DIR) / 'etc' / 'perms'
GROUP_PERMS_DIR = _PERMS_DIR / 'group'
USER_PERMS_DIR = _PERMS_DIR / 'user'
...
class Command(BaseCommand):
    ...
    def _get_create_groups(self):
        for i, item in enumerate(DEFAULT_GROUPS):
            group, created = Group.objects.get_or_create(defaults=item, id=item['id'])
            if created:
                self.stdout.write(
    ...
    def handle(self, *args, **options):
        ...
        for perm_file in USER_PERMS_DIR.iterdir():
            if perm_file.is_file() and perm_file.suffix.lstrip('.') in ('yml', 'yaml'):
                try:
                ...
                users.append(
                    {'user': user, 'user_perms': user_perms or {},
